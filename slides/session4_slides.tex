...
...
    ...
    \vspace{0.4em}
    \begin{enumerate}
      \item At session start, Claude sees every skill's \texttt{name} and \texttt{description} --- nothing more
      ...
      \item Claude follows those instructions for the current task
    \end{enumerate}

    \vspace{0.5em}
    \begin{block}{The key idea}
      The \texttt{description} is the trigger. Write it so Claude can tell \emph{when} this skill applies.
    \end{block}
  \end{columns}
\end{frame}

\begin{frame}{Why They Didn't Stick}
  \textbf{Looking back, what these four have in common:}

  \vspace{0.5em}
  \begin{itemize}
    \item Built \emph{on spec} --- ``this would be good to have'' --- not in response to pain
    \item Flavored toward \emph{generic discipline} --- ``good research practice''
    \item Not tied to a specific failure mode I actually hit
    \item Each could be replaced by a prompt the moment I needed it
  \end{itemize}

  \vspace{0.5em}
  The pattern is predictable once you see it. A skill built on an idea of how research \emph{should} go --- rather than on a pain I kept hitting --- doesn't survive contact with actual work.
...
% ---- 2e: Worked example: LaTeX table skill ----------------------

\begin{frame}{The One I Actually Use: \LaTeX{} Tables}
  \textbf{Failure mode it addresses:}

  \vspace{0.2em}
  Every table I generate needs two or three rounds of formatting before it's submission-ready. Same rounds, every time.

  \vspace{0.5em}
  \textbf{Conventions the skill encodes:}
  \begin{itemize}
    \item Significance notation (stars vs.\ p-values; thresholds; footnote placement)
    \item Standard-error style (robust, clustered, which level to display)
    \item Table-notes format (variable definitions, sample restrictions)
    \item Override conditions for journal-specific requirements
  \end{itemize}

  \vspace{0.4em}
  \begin{block}{Why this is a convention, not a prompt}
    Re-specifying these rules every time is expensive. The conventions are stable. They're about me, not about the paper.
  \end{block}
\end{frame}

\begin{frame}{Where It Helped (This Past Week)}
  ...
  \vspace{0.6em}
  \textbf{What the skill saved:}
  \begin{itemize}
    \item Roughly 15 minutes per table of formatting fixes
    \item Zero context-switching between ``what did I format like last time?'' and the actual analysis
    \item Consistency across tables in the same paper --- something I'd drift on manually
  \end{itemize}

  \vspace{0.5em}
  \alert{Small per-use savings. But tables compound --- every paper has many.}
\end{frame}

\begin{frame}{Where It Fell Short (And What I'd Change)}
  \textbf{Where it fired wrong:}
  \begin{itemize}
    \item Over-eager on notes --- added full table notes when I wanted the slide version
    \item Didn't know the target journal's specific significance conventions until I told it
  \end{itemize}

  \vspace{0.5em}
  \textbf{What I'd change:}
  \begin{itemize}
    \item Explicit trigger conditions: paper conventions vs.\ slide conventions
    \item A one-line journal-override mechanism at the top of the skill
  \end{itemize}

  \vspace{0.5em}
  \begin{alertblock}{The point}
    Skills are empirical artifacts. You build them, fail at them, refine them. The \LaTeX{} skill is on version three.
  \end{alertblock}
\end{frame}

% ---- 2f: Probationary skills ------------------------------------

\begin{frame}{Four More I've Built. Not Yet Trialed.}
  \textbf{Explicitly a different bucket from the \LaTeX{} skill:}

  \vspace{0.6em}
  \begin{enumerate}
    \item \textbf{LLM-mistake checker} --- catches the class of errors Claude systematically makes (silent NaN drops, wrong merge keys, ignored clustering)
    \item \textbf{Sanity-check validator} --- fires on freshly-loaded data to verify ranges, types, and counts match expectations
    \item \textbf{Testing-philosophy skill} --- prevents over-mocked tests that pass but don't catch real bugs
    \item \textbf{Paragraph-structure audit} --- one claim per paragraph, mis-cited claims, smuggled assumptions, weak transitions. The integrated version queries Zotero for candidate citations when claims are uncited.
  \end{enumerate}

  \vspace{0.6em}
  \alert{These I built. I'll trial each once before class. But none are yet validated by real use.}
\end{frame}

\begin{frame}{Why I Think These Might Stick}
  \textbf{What's different from the earlier four:}

  \vspace{0.4em}
  \begin{itemize}
    \item Each is tied to a \emph{specific LLM failure mode}, not generic ``good practice''
    \item Each is a \emph{verification skill} --- fires when there's a concrete question (``is this code right?'' ``is this data clean?''), not when I'm ambiently ``doing research''
    \item The trigger condition for each is sharp --- I can picture the moment I'd reach for it
  \end{itemize}

  \vspace{0.5em}
  \textbf{What I'm waiting to learn:}

  \vspace{0.2em}
  Whether they fire at the right moments --- or whether the description matching misses the cases that actually matter.

  \vspace{0.4em}
  \begin{block}{Place your bets}
    Which of the four survives three months? I'll report back in a later class.\\
    Skill adoption is empirical, not theoretical.
  \end{block}
\end{frame}

% ---- 2f2: Skill + MCP composition (teachable pattern) -----------
