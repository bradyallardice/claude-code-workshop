% Session 4: Advanced Claude Code --- Skills and MCPs
...
\begin{frame}{Where We Are}
  ...
  Today you will:
  \begin{enumerate}
    \item Run the usual workspace ritual --- routine since session 1, quick recap below
...
\begin{frame}[fragile]{Quick Recap: Your Workspace}
  \textbf{The layout you already know:} each session is a self-contained folder.
...
claude-code-workshop/
  ...
  \vspace{0.5em}
  \textbf{The workflow you've used since session 1:} copy the session folder out of the repo into your own workspace. Work there. The shared repo stays read-only for you.
...
\begin{frame}[fragile]{The Three-Step Ritual}
  \textbf{Same as every session --- let's get into today's clean state:}
...
cd ~/my_workspace/session_4 && code .
  ...
  \vspace{0.2em}
  \begin{alertblock}{If it says it can't find the path or rejects \texttt{\textasciitilde}}
    {\small \textbf{Type these by hand --- don't paste.} Pasting can turn \texttt{\textasciitilde} and \texttt{\_} into look-alikes the shell rejects; \texttt{mkdir} (step~2) creates \texttt{\textasciitilde/my\_workspace}. No spaces in the names. \textbf{On Windows? Use the PowerShell version next slide.}}
  \end{alertblock}
\end{frame}

\begin{frame}[fragile]{The Three-Step Ritual --- Windows (PowerShell)}
  \textbf{PowerShell isn't bash:} \texttt{\&\&}, \texttt{cp -r}, and \texttt{mkdir -p} don't work. Use these instead:
...
# 2. Make your workspace (first time only), then copy the folder in
mkdir ~\my_workspace -Force
Copy-Item -Recurse session_4 ~\my_workspace\session_4

# 3. Open your copy in VS Code and work there
cd ~\my_workspace\session_4
code .
  \end{lstlisting}

  \vspace{0.2em}
  {\small \textbf{One command per line} --- Windows PowerShell 5.1 doesn't support \texttt{\&\&}. The \texttt{git} commands are identical to macOS, so the recovery steps on the next slide work unchanged.}
\end{frame}
...
% ---- Live demo: crowdsource a skill ------------------------------

\begin{frame}{Live Demo: What Could Be a Skill?}
  \begin{alertblock}{Over to you}
    Think about your own research workflow. \textbf{What is something you do over and over the same way?} Shout one out --- we'll turn it into a skill, live.
  \end{alertblock}

  \vspace{0.5em}
  \textbf{What makes a good candidate:}
  \begin{itemize}
    \item You do it \textbf{repeatedly}, and roughly the \textbf{same way} each time
    \item There's a \textbf{house style} or rule you keep re-explaining to Claude
    \item You can describe \textbf{when} it applies in one sentence
  \end{itemize}

  \vspace{0.4em}
  {\small Examples to prime the room: formatting a table to your journal's spec, renaming survey variables to a convention, structuring a referee response, building a balance table, cleaning a messy export the same way every wave.}

  \vspace{0.4em}
  \begin{block}{The plan}
    \footnotesize Pick one. We write the \texttt{description} together, sketch the \texttt{SKILL.md} body, and watch Claude invoke it.
  \end{block}
\end{frame}

% ---- 2b: The architecture ---------------------------------------

\begin{frame}[fragile]{Where Skills Live: The Hierarchy, Again}
  Remember the user $\rightarrow$ project $\rightarrow$ subfolder hierarchy from CLAUDE.md in Session 1? \textbf{Skills follow the same pattern.}
...
~/.claude/skills/latex-tables/
  SKILL.md
\end{lstlisting}
    {\small Available in \emph{every} project. For things stable \emph{about you}.}
...
your-project/.claude/skills/
  paper-style/
    SKILL.md
\end{lstlisting}
    {\small Loaded only in this project. Checked into git, shared with collaborators.}
  \end{columns}

  \vspace{0.6em}
  \begin{block}{The same hierarchy keeps showing up}
    \textbf{Hooks}, \textbf{MCP servers and permissions}, and \textbf{subagents} all live at either the user or project level too. The decision is always the same: \emph{is this stable about me, or specific to this project?}
  \end{block}
\end{frame}
...
git push -u origin chapter-N
  ...
  \vspace{0.1em}
  {\small \textbf{Windows:} every line above works the same in PowerShell --- no \texttt{\&\&}, no \texttt{cp}/\texttt{mkdir}, and \texttt{\#} starts a comment in both shells.}
...
\begin{frame}{MCP as Helper, Not Substitute}
  ...
  \begin{alertblock}{The risk the MCP unlocks}
    The GitHub MCP can approve and merge PRs autonomously. Say ``handle the PR'' and Claude can push to \texttt{main} with no human review --- fine for a playful class story, dangerous on a real research repo.
  \end{alertblock}

  \vspace{0.3em}
  \textbf{How to stay safe:}
  \begin{itemize}\small
    \item \textbf{Turn on branch protection} --- a GitHub setting (Repo $\rightarrow$ Settings $\rightarrow$ Branches) that won't merge a PR without an approval from \emph{someone other than the author}. The MCP has to obey it too.
  ...
  \vspace{0.3em}
  \alert{\small Asking for two humans was a \emph{convention}, not a GitHub rule --- branch protection is what makes review mandatory.}
\end{frame}
...
% ---- Working with coauthors: Overleaf bridge --------------------

\begin{frame}{If Your Coauthors Are on Overleaf}
  Moving your paper into VS Code does not mean abandoning your coauthors.

  \vspace{0.8em}
  \textbf{Overleaf supports git sync.} You can connect your Overleaf project to a git remote and push/pull like any other repository.
...
\begin{frame}{What Claude Code Can Do Across the Collaboration}
  Once your \texttt{.tex} file is local, Claude Code can operate on the whole project together:

  \vspace{0.8em}
  \begin{itemize}
    \item \textbf{Consistency checks:} does the methods section match the regression code?
    \item \textbf{Table updates:} rerun the analysis, regenerate the \LaTeX{} table, commit in one step
    \item \textbf{Revision responses:} paste a referee comment, ask Claude Code to find the relevant code and text
    \item \textbf{Version history:} every Claude Code edit is a commit --- you can show coauthors exactly what changed and why
  \end{itemize}

  \vspace{0.8em}
  \begin{block}{The Payoff}
    Your coauthors do not need to change anything.\\
    You gain a workflow where your paper, code, and data are always in sync.
  \end{block}
\end{frame}
...
\begin{frame}{Coming Next Session}
  ...
  \vspace{1.2em}
  \begin{flushleft}
  \hspace{2em}\textbf{1. Subagents} --- a focused Claude spawned for a bounded job: parallelism plus context hygiene, then scaled up into named, reusable specialists.
  ...
  \vspace{0.6em}
  \hspace{2em}\textbf{3. The final project} --- two-person teams create an empty GitHub repo, copy in \texttt{session\_5/capstone-template/}, read the paper, use Zotero, then write a short replication + extension note.
  \end{flushleft}
\end{frame}
...
\begin{frame}[shrink=10]{Next Session + Prework}
  \textbf{Session 5: Subagents, Hooks, and the Final Project.}

  \vspace{0.6em}
  \begin{itemize}
    \item \textbf{Subagents} --- when to spawn one, then how to define named, reusable specialists with their own scope and tools
    \item \textbf{Hooks} --- automatic shell commands on events (file writes, tool calls, session start)
    \item \textbf{Final project} --- two-person teams create an empty GitHub repo, copy in \texttt{session\_5/capstone-template/}, then do paper familiarization, Zotero-backed literature review, replication, extension, and reflection
  \end{itemize}

  \vspace{0.8em}
  \textbf{Before next session:}
  \begin{enumerate}
    \item \textbf{Check GitHub:} make sure you can create a repo, add a collaborator, and push a branch
    \item \textbf{Check Zotero:} Session 5 imports \texttt{zotero/capstone\_library.bib} with PDFs from \texttt{zotero/pdfs/}
    \item \textbf{Run the routing test} on one thing you wished Claude already knew --- is it a skill, project context, MCP, hook, subagent, or just a one-off prompt?
