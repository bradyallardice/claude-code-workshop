...
cd ~/my_workspace/session_4 && code .
  ...
  \begin{alertblock}{If it says it can't find the path or rejects \texttt{\textasciitilde}}
    {\small \textbf{Type the commands by hand --- don't paste from these slides.} Pasting can turn \texttt{\textasciitilde} (home) and \texttt{\_} into look-alike characters the shell rejects. Step~2's \texttt{mkdir} creates \texttt{\textasciitilde/my\_workspace} so the copy has somewhere to land. \textbf{No spaces} in \texttt{session\_4} or \texttt{my\_workspace}.}
  \end{alertblock}
\end{frame}
