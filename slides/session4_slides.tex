...
\begin{frame}[fragile]{GitHub's Answer: Branches + PRs}
  ...
  \begin{tikzpicture}[
    ...
    \node[box] (c) at (-2.4, 0.3) {commit};
    \node[box] (i) at (-0.8, 0.3) {instruct};
    \node[box] (d) at (0.8, 0.3) {diff};
    \node[box] (k) at (2.4, 0.3) {keep/revert};
    \draw[arr] (c) -- (i);
    \draw[arr] (i) -- (d);
    \draw[arr] (d) -- (k);
...
\begin{frame}[fragile,label=troubleshooting]{Troubleshooting: When GitHub Fights Back}
  ...
  \vspace{0.3em}
  \begin{itemize}\small
    \item \textbf{Authentication fails on first push.} \\
    Run \texttt{gh auth login} and pick HTTPS + browser. Install with \texttt{brew install gh} if you don't have it.
...
git pull --rebase origin main && git push
    ...
    \item \textbf{Conflict markers (\texttt{<<<<<<<}) in a file.} \\
    Open it, edit to the version you want, delete the markers. Then \texttt{git add <file>} and \texttt{git rebase --continue} (or \texttt{git commit} if you were merging).

    \item \textbf{You committed to \texttt{main} by accident.} \\
    Save the work to a new branch, then rewind \texttt{main}:
    \begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
git checkout -b chapter-N
...
git checkout chapter-N
    ...
    \item \textbf{PR opened against the wrong base.} \\
