...
\begin{frame}{First: What Is GitHub?}
  \begin{columns}[T]
    ...
    \vspace{0.5em}
    Every project is a \emph{repository} (``repo'') --- a folder with all its files, plus the full history of every change.
    ...
    \vspace{0.4em}
    \textbf{Changes flow between them:}
    \begin{itemize}
      \item \texttt{clone} --- first copy of the remote onto your machine
      \item \texttt{pull} --- fetch the latest remote changes
      \item \texttt{push} --- send your local changes to the remote
      \item \textbf{branches} + \textbf{PRs} --- how people propose and merge work
    \end{itemize}

    \column{0.34\textwidth}
    \centering
    \vspace{1.5em}
    \includegraphics[width=0.65\textwidth]{img/github.png}
  \end{columns}

  \vspace{0.4em}
  \begin{block}{The short version}
    GitHub hosts the \emph{shared} copy. You work on your \emph{local} copy. Git is the tool that keeps them in sync.
  \end{block}
\end{frame}
...
\begin{frame}{Part 2: Skills}
  \centering
  \vspace{2em}
  {\Large Most of the skills people build\\[0.3em]
  get abandoned.}

  \vspace{1em}
  {\Large Let's figure out which ones survive.}
