...
\begin{frame}[fragile]{This Is Not Reliable}
  \textbf{Reasonable assumption:} \texttt{.claudeignore} is to Claude Code what \texttt{.gitignore} is to git. Every one of those posts says so.

  \vspace{0.8em}
  I created the file and tested it. Claude Code read right through it --- \texttt{Read}, \texttt{Grep}, and \texttt{Glob} all returned the ``hidden'' files.

  \vspace{0.8em}
  \alert{\texttt{.claudeignore} is not a real feature.} Nobody reads the actual docs; one person writes a confident post; everyone cites it. \textbf{An internet-wide hallucination.}

  \vspace{0.8em}
  It can \emph{appear} to work when Claude happens not to touch the file --- indistinguishable from no ignore at all. That's how the story stays alive.
\end{frame}

\begin{frame}[fragile]{The ``Official'' Answer Doesn't Work Either}
  ...
  \begin{lstlisting}
{
  ...
  \vspace{0.2em}
  \textbf{But:} it only blocks Claude's \texttt{Read}/\texttt{Grep}/\texttt{Glob}. A \texttt{Bash(cat file)} reads the file anyway. Claude will route around the fence the moment it needs to.

  \vspace{0.6em}
  \textbf{Coming later:} when we cover \alert{hooks}, we'll see a mechanism that actually stops the model from reaching what you want hidden.
\end{frame}
