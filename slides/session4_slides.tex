  ...
  \begin{lstlisting}
{
  ...
  \vspace{0.2em}
  \textbf{The catch:} it blocks \texttt{Read}/\texttt{Grep}/\texttt{Glob}, not \texttt{Bash}. A \texttt{cat} or \texttt{head} in a shell command reads the file anyway.

  \vspace{0.6em}
  \textbf{The honest version:} on emerging tools, multiple paths look reasonable and each has gaps. Know the gaps, combine with care.

  \vspace{0.4em}
  \alert{For now, the only safe assumption is: if you put it in the project, Claude can read it. Be deliberate about what you give it access to.} We'll see a stronger mechanism when we cover \alert{hooks}.
\end{frame}
...
...
    ...
    \begin{enumerate}\small
      ...
      \item \textbf{Two ways to invoke:} Claude auto-calls when a task matches the description, OR you invoke it explicitly (e.g. \texttt{/latex-tables})
      \item The full body of \texttt{SKILL.md} loads into context on demand
      \item Claude follows those instructions
    \end{enumerate}

    \vspace{0.3em}
    \begin{block}{\small The key idea}
      \small The \texttt{description} is the trigger for auto-calls. Write it so Claude can tell \emph{when} this skill applies --- or invoke it yourself when you know you want it.
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{Finding Good Use Cases Is Hard}
  \textbf{Four skills I built but barely use:}

  \vspace{0.3em}
  \begin{itemize}
    \item \texttt{auto-commit} --- commits after every file edit
    \item \texttt{research-docs} --- maintains project documentation
    \item \texttt{robustness-checks} --- structured robustness-spec battery
