...
\begin{frame}{The Dangers of Emerging Technologies}
  ...
  \centering
  \begin{tikzpicture}
    \node[rotate=-7] at (-5.0, 0.9) {\includegraphics[width=5cm]{img/git_ignore.png}};
...
\begin{frame}[fragile]{I Tried It. It Didn't Work.}
  \textbf{Reasonable assumption:} \texttt{.claudeignore} is to Claude Code what \texttt{.gitignore} is to git.

  \vspace{0.8em}
  I created the file. Claude Code read right through it --- \texttt{Read}, \texttt{Grep}, and \texttt{Glob} all returned the ``hidden'' files.

  \vspace{0.8em}
  The blog posts and Reddit threads are half-right at best:
  \begin{itemize}
    \item \texttt{.claudeignore} is not a real feature.
    \item It \emph{sometimes} appears to work --- when Claude happens to not look at the file, which is indistinguishable from doing nothing.
    \item There are dozens of community ``workarounds'' circulating. Most add a gitignore entry and hope.
  \end{itemize}

  \vspace{0.8em}
  \alert{Emerging tools invite confident guessing by analogy. The analogy lies.}
\end{frame}

\begin{frame}[fragile]{The Official Answer --- for Now}
  Hide files from Claude's read tools via \texttt{permissions.deny} in \texttt{.claude/settings.json}:
  ...
  \begin{lstlisting}
{
  ...
  \vspace{0.2em}
  \textbf{Caveat:} deny rules don't block Bash --- \texttt{cat file} still reads it.

  \vspace{0.6em}
  \textbf{Coming later:} when we cover \alert{hooks}, we'll see a cleaner mechanism for controlling what Claude touches and when.
\end{frame}
