...
# 5. Push to GitHub (first time uses -u; later pushes: git push)
...
git checkout my-branch-name   # switch to an existing branch
  ...
  \alert{\small Commit before switching. Uncommitted edits follow you between branches.}
\end{frame}
...
\begin{frame}[fragile,label=troubleshooting]{Troubleshooting: When GitHub Fights Back}
  ...
  \begin{itemize}\footnotesize
    ...
    \item \textbf{Conflict markers (\texttt{<<<<<<<}) in a file.} Open it, edit to the version you want, delete the markers. Then \texttt{git add <file>} and \texttt{git rebase -{}-continue} (or \texttt{git commit} if merging).
