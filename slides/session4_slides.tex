...
\begin{frame}{Coming Next Session}
  \centering
  \vspace{2em}
  {\Large Session 5: \textbf{Building a team of specialized agents}.}

  \vspace{1em}
  {\large Named subagents you define once and invoke on demand ---\\[0.3em]
  each with its own scope, tools, and system prompt.}

  \vspace{1.5em}
  A \texttt{lit-search} agent. A \texttt{robustness-check} agent. A \texttt{draft-reviewer} agent.\\[0.3em]
  Running in parallel when the work lets them.
\end{frame}
