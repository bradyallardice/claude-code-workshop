...
\begin{frame}{Pull Requests: From Branch to Main}
  \textbf{Your branch is pushed. A PR is the proposal to merge it into \texttt{main}.}

  \vspace{0.5em}
  \textbf{Drafter --- open the PR on GitHub}
  \begin{enumerate}
    \item After you push, GitHub shows a yellow banner: \textbf{``Compare \& pull request''}. Click it.
    \item Check the \textbf{base} is \texttt{main} and the \textbf{compare} is your branch
    \item Write a short title and description
    \item Assign your partner as reviewer, then \textbf{Create pull request}
  \end{enumerate}

  \vspace{0.5em}
  \textbf{Reviewer --- on the PR page}
  \begin{enumerate}
    \item Click \textbf{Files changed} to see the diff
    \item \textbf{Review changes} $\rightarrow$ \textbf{Approve}
    \item Once approved: \textbf{Merge pull request} $\rightarrow$ \textbf{Confirm merge}
  \end{enumerate}

  \vspace{0.4em}
  \alert{\small Nothing in \texttt{main} changes until someone clicks Merge. That's the point --- review happens first.}
\end{frame}
...
\begin{frame}{Exercise: The Collaborative Story}
  ...
  \vspace{0.5em}
  \alert{Once you vote, I'll push the cast of characters for your story to \texttt{main}. Then you clone and write.}
\end{frame}
