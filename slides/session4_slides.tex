...
\begin{frame}[fragile]{\texttt{.claudeignore}: Works, Sometimes}
  The natural guess: \texttt{.claudeignore} is to Claude Code what \texttt{.gitignore} is to git. A lot of the internet says so.

  \vspace{0.8em}
  What the testing actually shows:
  \begin{itemize}
    \item If you tell Claude \emph{explicitly} to respect it, it usually does.
    \item If you don't --- even if it has just read \texttt{.claudeignore} and seen the pattern --- it will read the file anyway.
    \item There's no tool-level enforcement. It's a polite request that only lands when the model is listening.
  \end{itemize}

  \vspace{0.6em}
  So it ``works'' the way a sticky note on someone's monitor works: useful, sometimes, for people who happen to read it.
\end{frame}
