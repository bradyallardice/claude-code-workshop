...
\begin{frame}[fragile]{What Ignore Files Look Like}
  \begin{columns}
...
__pycache__/
  ...
  \begin{itemize}
    \item \texttt{*.pyc} = all .pyc files
    \item \texttt{data.*.csv} = data.raw.csv, data.clean.csv, etc.
    \item \texttt{analysis_*.py} = analysis_v1.py, analysis_v2.py, etc.
  \end{itemize}
  \textbf{Folders} must end with \texttt{/} (e.g., \texttt{output/}, \texttt{__pycache__/})}
\end{frame}
