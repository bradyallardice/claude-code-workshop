...
\begin{frame}{Exercise Part 1: Initialize and Commit (10 min)}
  ...
  \begin{enumerate}
    \item \textbf{Create a \texttt{.gitignore}} for your project (Ask Claude Code to help):
    \begin{itemize}
    ...
    {\small \alert{Important:} If using Option B, you will find a \texttt{.env} file in the sandbox. These are my real credentials. If you forget to add \texttt{.env} to \texttt{.gitignore} or \texttt{.claudeignore}, I will have to change them. Don't do that.}

    {\small \textbf{Bonus:} Create a \texttt{.claudeignore} file using the exact same syntax to keep sensitive files away from Claude Code.}
