...
\begin{frame}{What I Want to Know}
  \begin{itemize}
...
\begin{frame}{Exercise Part 3: Build a \LaTeX{} Document (30 min)}
  \textbf{Choose your adventure:}
  \begin{itemize}
    \item \textbf{Option A:} Use \textbf{your own work} --- a current or recent research project with tables and figures you already have
    \item \textbf{Option B:} Use the \textbf{provided project} (\texttt{module\_3/}) with FX data, summary stats, main table, and robustness table
  \end{itemize}

  \vspace{1em}
  \textbf{Your goal:} Build out two sections of a paper --- \textbf{Data} and \textbf{Results}.
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item Include one descriptive figure\\
      (Option B: the FX exchange rate over time, \texttt{fx\_rate\_figure.png})
    \end{itemize}
  \end{enumerate}
\end{frame}

\begin{frame}{Exercise Part 3: Build a \LaTeX{} Document (cont.)}
  \begin{enumerate}
    \setcounter{enumi}{2}
    \item \textbf{Results section:} Ask Claude Code to include three tables, each with a paragraph of text describing what it shows:
    \begin{itemize}
      \item Summary statistics (\texttt{summary\_stats.tex})
      \item Main results (\texttt{main\_table.tex})
      \item Robustness (\texttt{robustness\_table.tex})
    \end{itemize}
    ...
    \vspace{0.6em}
    \item \textbf{Ask Claude Code} to check your text against the actual code. Does what you wrote match what the code does?
  \end{enumerate}

  \vspace{1em}
  \alert{You are now using git and \LaTeX{} together.} Every change to the paper is tracked.
\end{frame}
