...
\begin{frame}[fragile]{The Minimum Viable Git Workflow}
  ...
  \begin{tikzpicture}[
    ...
    box/.style={draw=anthropic, fill=lightgray, rounded corners, minimum width=3.5cm, minimum height=0.8cm, align=center, font=\small\ttfamily},
    ...
    \draw[arr, rounded corners=12pt] (decide.south) -- ++(0,-0.5) -| (commit.south);
  \end{tikzpicture}
  \end{center}
  ...
  \begin{tabular}{ll}
    ...
    \texttt{git diff} & See what changed since the last commit \\
...
\begin{frame}[fragile]{Step 1: Commit Before You Instruct}
  \textbf{Always save your working state before Claude Code touches anything.}
...
git commit -m "Working merge script before adding controls"
  ...
  \begin{itemize}
    \item A snapshot you can always return to
    \item A clear ``before'' to compare against
    \item Permission to let Claude Code experiment --- you can always undo
  \end{itemize}
...
\begin{frame}[fragile]{Step 2: Review the Diff}
...
git diff
  ...
  \begin{itemize}
    \item Files you did not ask it to change
    \item Lines removed that should not have been
    \item ``Helpful'' additions you did not request
    \item Changes that look correct but are in the wrong file
  \end{itemize}

  \vspace{0.8em}
  \textbf{You can also ask Claude Code to explain the diff:}
  \begin{lstlisting}
Run git diff and explain each change you made.
  \end{lstlisting}

  \vspace{0.5em}
  \alert{Read the diff, not just the output.} The diff shows you what actually changed in your project.
\end{frame}

\begin{frame}[fragile]{Step 3: Keep or Revert}
  \textbf{If everything looks good:}
  \begin{lstlisting}
git add -A
git commit -m "Add demographic controls to specification"
  \end{lstlisting}

  \vspace{0.8em}
  \textbf{If something is wrong --- revert a specific file:}
  \begin{lstlisting}
git checkout -- scripts/analysis.py
  \end{lstlisting}

  \vspace{0.5em}
  \textbf{If everything is wrong --- revert all changes:}
  \begin{lstlisting}
git checkout -- .
  \end{lstlisting}

  \vspace{0.8em}
  \begin{alertblock}{The 30-Second Safety Net}
    This entire cycle --- commit, instruct, diff, decide --- takes 30 seconds of overhead. That is the cost of never losing work.
  \end{alertblock}
\end{frame}
