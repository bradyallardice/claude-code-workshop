...
\begin{frame}[fragile]{Start-of-Session Ritual}
  \textbf{Same as sessions 1 and 2:} work on a copy, not inside the shared repo. Do this now:
...
\begin{frame}{Exercise Part 3: Choose Your Adventure}
  \begin{itemize}
    \item \textbf{Option A:} Use \textbf{your own work} --- a current or recent research project with data, code, and outputs you already have
    \item \textbf{Option B:} Use the \textbf{provided project} (\texttt{module\_5/option\_b/}) with data, scripts, and a skeleton to fill in
  \end{itemize}

  \vspace{1em}
  \textbf{Two exercises:}
  \begin{enumerate}
    \item \textbf{Exercise 1:} Practice git --- modify scripts, create a new one, generate your outputs
    \item \textbf{Exercise 2:} Use those outputs to build a \LaTeX{} paper
  \end{enumerate}

  \vspace{1em}
  \alert{Commit $\rightarrow$ instruct $\rightarrow$ diff $\rightarrow$ decide.} You should be committing throughout both exercises.
\end{frame}

\begin{frame}{Exercise 1: Practice Git, Generate Outputs (20 min)}
  \textbf{Option B:} You have three scripts in \texttt{module\_5/option\_b/exercise\_1/}:
  \begin{itemize}
    \item \texttt{solution\_merge.py} --- merges survey with demographics
    \item \texttt{solution\_estimate.py} --- main regression, summary stats
    \item \texttt{solution\_robustness.py} --- robustness checks
  \end{itemize}

  \vspace{0.6em}
  \textbf{Your tasks:}
  \begin{enumerate}
    \item \textbf{Commit} your starting state
    \item \textbf{Modify} something in \texttt{solution\_estimate.py} or \texttt{solution\_robustness.py} --- add a variable to the summary table, change a specification, rename something. \textbf{Diff, commit, revert} as you go.
    \item \textbf{Ask Claude Code} to create a new \texttt{generate\_fx\_figure.py} script that plots the CHF/PLN exchange rate from \texttt{data/PLN\_CHF\_EUR\_FXdata.csv} over time
    \item \textbf{Run all four scripts} --- outputs should appear in \texttt{exercise\_2/tables/} and \texttt{exercise\_2/figures/}
    \item \textbf{Commit} the generated outputs
  \end{enumerate}
\end{frame}

\begin{frame}{Exercise 2: Build the Paper (30 min)}
  Your goal: build two sections --- \textbf{Data} and \textbf{Results} --- using the outputs you just generated.

  \vspace{0.5em}
  \begin{enumerate}
    \item \textbf{Data section:} Ask Claude Code to
    \begin{itemize}
      \item Write text describing the data
      \item Include your FX figure via \texttt{\textbackslash includegraphics\{figures/fx\_rate\_figure.png\}}
    \end{itemize}

    \vspace{0.3em}
    \item \textbf{Results section:} Include three tables, each with a paragraph of text:
    \begin{itemize}
      \item Summary statistics, main results, robustness
      \item Pull them in with \texttt{\textbackslash input\{tables/...\}}
    \end{itemize}

    \vspace{0.3em}
    \item \textbf{Compile} to PDF, fix errors with Claude Code, then \textbf{diff and commit}

    \vspace{0.3em}
    \item \textbf{Ask Claude Code} to check your text against your (possibly modified) scripts. Does the paper match what the code actually does?
  \end{enumerate}
\end{frame}
...
\begin{frame}{Looking Ahead: Session 5}
  \textbf{Tentative plan:} Writing a paper end-to-end with Claude Code.

  \vspace{0.8em}
  Take everything from Sessions 1--4 --- data cleaning, estimation, robustness, \LaTeX{}, git, hooks, Zotero --- and use it to produce a full draft from a blank \texttt{.tex} file to a compiled PDF.

  \vspace{1em}
  \textbf{But I want your input.}
  ...
  \vspace{0.5em}
  \begin{itemize}
    \item A topic we rushed through that deserves more time?
    \item A use case you have not been able to figure out on your own?
    \item Something you have tried with Claude Code that did not work the way you expected?
  \end{itemize}
