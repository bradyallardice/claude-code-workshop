...
\begin{frame}[fragile]{Restricting Claude to This Project}
  In VS Code, file access is controlled via \texttt{.claude/settings.json} in your project root.\\[0.5em]
  This course ships with this file already configured:
  ...
  \begin{lstlisting}[language=json, basicstyle=\ttfamily\small]
{
  ...
  \begin{itemize}
    \item Blocks reads and writes outside the project directory.
    \item Allows full access \textit{within} the project.
    \item Still allows Claude to load your user-level config (\texttt{\textasciitilde/.claude/}).
  \end{itemize}
\end{frame}
