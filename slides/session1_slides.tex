...
\begin{frame}{Dialing the Effort}
  ...
  \vspace{0.5em}
  \textbf{Effort, on Opus.} Opus also has an \textbf{effort} setting (low / medium / high) you can set with \texttt{/model}. Higher effort = more thorough, slower, costlier. Think of it as the same knob, set once for the session rather than per-prompt.
...
\begin{frame}{Getting Files In: Drag and Drop}
  ...
  \vspace{0.6em}
  \textbf{Drag into the chat panel} (in VS Code) to add something as context:
  \begin{itemize}
  ...
  \begin{alertblock}{Two ways to do it}
    \begin{itemize}
      \item \textbf{Drag} a file from the VS Code Explorer (or your Finder/desktop) onto the chat box
