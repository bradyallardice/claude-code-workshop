...
\begin{frame}[fragile]{Get the Course Code}
...
\end{lstlisting}
  ...
  \vspace{0.5em}
  This downloads the course repository to your computer. Then open the folder in VS Code:

  \vspace{0.3em}
  \textbf{File $\rightarrow$ Open Folder $\rightarrow$} select \texttt{claude-code-workshop}

  \vspace{0.8em}
  \textbf{Already cloned it?} Just run \lstinline{git pull} from inside the folder to get the latest version.
\end{frame}
...
\begin{frame}{Exercise: Your First Commands}
  ...
  \vspace{0.8em}
  \textbf{Task 2:}\\
  Ask Claude Code to add verification checks, documentation, and print statements to the script.

  \vspace{0.8em}
  \textbf{Task 3:}\\
  Ask Claude Code to change the two-party vote shares to total vote shares.
\end{frame}
...
\begin{frame}{Where Claude Looks for Context}
  ...
  \begin{itemize}
    ...
    \item \textbf{Local overrides} --- \texttt{CLAUDE.local.md}\\
      {\small Gitignored. Personal notes that should not be shared with collaborators.}
  \end{itemize}

  \vspace{0.4em}
  \begin{block}{How Claude reads them}
    All applicable files are concatenated into context. More specific (deeper) files appear later, so they take precedence when guidance conflicts.
  \end{block}
\end{frame}
...
\begin{frame}{What Belongs in CLAUDE.md}
  ...
  \vspace{0.6em}
  {\small Keep the \textit{user-level} CLAUDE.md for things that apply across projects (your role, preferred language, default style). Use a \textit{subfolder} CLAUDE.md only when a directory needs its own rules.}

  \vspace{0.6em}
  \begin{block}{How it interacts with the context window}
    Every applicable CLAUDE.md is loaded before your first message and stays ``visible'' for the whole session --- but it takes up space, so keep each one focused and concise.
  \end{block}
\end{frame}
