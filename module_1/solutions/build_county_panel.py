...
Reads:
  - ../data/countypres_sample.csv (MIT Election Data + Science Lab, filtered)
  - ../data/IPUMS/census_{year}_sample.csv.gz (IPUMS USA microdata, 10% sample)

Outputs:
  - ../data/county_panel.csv

The script aggregates person-level IPUMS records to county-level demographics
using person weights, and merges with county-level vote shares. Only counties
...
# Map each election year to the IPUMS sample file
ELECTION_TO_IPUMS = {
...
def load_ipums_county(year):
    """Load IPUMS sample microdata for a given year and aggregate to county level.

    Drops observations with COUNTYFIP == 0 (county not identified).
    Constructs 5-digit FIPS from STATEFIP and COUNTYFIP.

    Sample data columns: YEAR, STATEFIP, COUNTYFIP, PERWT, AGE, EDUC, INCWAGE, RACE, HISPAN
    """
    ...
    # Education (EDUC: 0-11 scale; 10+ = bachelor's+)
    ...
    # Income (INCWAGE: wage income; 999999 = N/A)
