...
3. Merge election and census data on county FIPS and year. The datasets have different county coverage per year — not every county appears in every IPUMS file. After the inner merge, filter the panel to only counties that appear in all 4 years (balanced panel). Save the result to `module_1/output/county_panel.csv`.
