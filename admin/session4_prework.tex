...
\section*{Step 1: Refresh the Course Repo}

I have pushed new material into \texttt{session\_4/}. Pull it down so you have it locally.

Open a terminal, navigate to wherever you cloned the course repo, and run:
\cmd{git pull}

You should see new files under \texttt{session\_4/}. If \texttt{git pull} fails with a merge conflict, that is exactly the problem we fix in class --- don't try to resolve it. Just email me and I will help you reset cleanly.
...
\alert{If I don't have your username before Tuesday evening, you won't be able to participate in the exercise.}
...
In your terminal, run (replace \texttt{YOUR\_PAT} with the token you just copied):
...
\subsection*{5a. Make sure you have a Zotero account with items in it}

\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item If you don't have one, create a free account at \textbf{https://www.zotero.org/user/register}
  \item Install the \textbf{Zotero desktop app} from \textbf{https://www.zotero.org/download/} if you don't already have it --- the MCP needs the desktop app running to read your local library (including full-text PDFs)
  \item Open the desktop app, sign in, and confirm you have \textbf{at least 5--10 items} in your library --- if it's empty, the demo has nothing to search. Drag a few PDFs in or use the browser connector to grab a handful of papers
\end{itemize}
...
\subsection*{5c. Install the \texttt{zotero-mcp} package}
...
\subsection*{5d. Add the Zotero MCP to Claude Code}

Run (substituting your real values from Step 5b):
...
\subsection*{5e. Confirm it works}

Open Claude Code in any directory and ask:
\cmd{Use the Zotero MCP to list 5 items from my library.}

If it returns a list of titles you recognize, you are done.
...
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  ...
  \item Asking Claude Code ``list 5 items from my Zotero library'' returns titles
