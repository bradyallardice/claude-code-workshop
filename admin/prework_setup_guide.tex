...
\section*{Step 3: Install Python and Required Packages}
...
\section*{Step 4: Install R (if you don't already have it)}
...
\section*{Step 8: Set Up Zotero}
In Session 4 we connect Claude Code to your Zotero library so it can search your references, pull metadata, and generate \texttt{.bib} entries for your \LaTeX{} document. To do that you need a Zotero account, the desktop app, a small library to search, and an API key.

\subsection*{Create a Zotero account and install the desktop app}
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item Go to \textbf{https://www.zotero.org/user/register} and create a free account
  \item Download Zotero from \textbf{https://www.zotero.org/download/} (Mac or Windows) and install it
  \item Open Zotero, sign in via \texttt{Edit > Settings > Sync} (Windows) or \texttt{Zotero > Settings > Sync} (Mac)
\end{itemize}

\subsection*{Populate your library}
For the Session 4 exercise to be useful, you need at least \textbf{10--20 references} in your Zotero library on a topic you care about (your own research, a literature you're starting to read, anything). If your library is empty:
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item Install the \textbf{Zotero Connector} browser extension from \textbf{https://www.zotero.org/download/}
  \item Use it to grab 10--20 papers from Google Scholar, a journal page, or a syllabus
\end{itemize}

\subsection*{Generate a Zotero API key}
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item Go to \textbf{https://www.zotero.org/settings/keys} (sign in if prompted)
  \item Click \textbf{Create new private key}
  \item Give it a name like \texttt{Claude Code MCP}
  \item Check \textbf{Allow library access} and \textbf{Allow notes access}; leave write permissions \textbf{enabled} (we will use the MCP to create annotations and notes)
  \item Click \textbf{Save Key} and \textbf{copy the key somewhere safe} --- you will not be able to see it again
  \item On the same page, note your \textbf{userID} (the number shown at the top, e.g. \texttt{userID: 1234567})
\end{itemize}

\textit{We will plug the API key and userID into the Zotero MCP configuration during Session 4 --- you do not need to install the MCP server in advance.}
...
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  ...
  \item You have a GitHub account (we will collect usernames on Day 1)
  \item Zotero desktop opens, your library has at least 10--20 items, and you have saved your API key and userID somewhere safe
\end{itemize}

\section*{Optional: Make VS Code Feel Like RStudio}
If you primarily work in R and would rather not give up the RStudio layout (script editor, console, environment, plots, help), VS Code can be configured to look and behave very similarly. \textbf{This is entirely optional} --- the workshop does not depend on it, and Claude Code works identically either way. Skip this section if you do not use R or are happy with the default VS Code layout.

\subsection*{Install the R packages VS Code needs}
Open R (in Terminal/PowerShell type \texttt{R}, or open the R GUI) and run:
\cmd{install.packages(c("languageserver", "httpgd", "jsonlite", "rlang"))}
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item \textbf{languageserver} powers code completion, hover help, and go-to-definition
  \item \textbf{httpgd} drives the inline plot viewer (the equivalent of RStudio's Plots pane)
\end{itemize}

\subsection*{Install Radian (a better R console)}
...
\cmd{pip install -U radian}
\plat{Mac:} If \texttt{pip} is not found, use \texttt{pip3 install -U radian}.

Find where Radian was installed --- in the same terminal, run \texttt{which radian} (Mac) or \texttt{where radian} (Windows) and copy the path.

\subsection*{Install the extra VS Code extension}
Open VS Code, go to Extensions (Cmd+Shift+X / Ctrl+Shift+X), and install:
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item \textbf{R Debugger} (by RDebugger) --- step-through debugging for R scripts
\end{itemize}
\textit{The main R extension is already installed from Step 1.}

\subsection*{Configure VS Code settings}
Open the Settings JSON file: press Cmd+Shift+P (Mac) / Ctrl+Shift+P (Windows), type \texttt{Preferences: Open User Settings (JSON)}, and add the following entries inside the outer braces (paste the path from the Radian step into \texttt{r.rterm.mac} or \texttt{r.rterm.windows}):
\cmd{"r.rterm.mac": "/usr/local/bin/radian",\\"r.rterm.windows": "C:\textbackslash\textbackslash Users\textbackslash\textbackslash YOU\textbackslash\textbackslash AppData\textbackslash\textbackslash Local\textbackslash\textbackslash Programs\textbackslash\textbackslash Python\textbackslash\textbackslash Python312\textbackslash\textbackslash Scripts\textbackslash\textbackslash radian.exe",\\"r.bracketedPaste": true,\\"r.alwaysUseActiveTerminal": true,\\"r.plot.useHttpgd": true,\\"r.sessionWatcher": true}

\subsection*{Open the RStudio-style layout}
Open any \texttt{.R} script in VS Code. With the script focused, press Cmd+Shift+P / Ctrl+Shift+P and run \textbf{R: Create R terminal} --- a Radian console appears at the bottom. Then open the R Workspace sidebar by clicking the R icon in the activity bar on the left (it appears once you open an \texttt{.R} file). You now have:
\begin{itemize}[leftmargin=2em,itemsep=0pt]
  \item Script editor (top) --- like RStudio's source pane
  \item R console (bottom) --- like RStudio's console
  \item R Workspace sidebar (left) --- like RStudio's environment + history
  \item Plot viewer (opens on first \texttt{plot()} call via httpgd) --- like RStudio's plots pane
\end{itemize}
...
\textbf{``command not found'' or ``zsh: bad pattern: \#'' after pasting from this PDF:} If you copy a multi-line block (anything with a \texttt{\#} comment, a blank line, or a line break) and paste it into the terminal, the shell will try to run each line as a separate command and choke on the comment or stray characters. \textbf{Paste one line at a time}, or strip the \texttt{\#} comments before pasting. Copying from a PDF can also turn straight quotes (\texttt{"}) into curly quotes (\texttt{\textquotedblleft\textquotedblright}), which the shell does not understand --- if a command with quotes fails, retype the quotes manually in the terminal.
...
\textbf{Zotero: lost the API key:} You cannot retrieve a saved key. Delete the old one at \textbf{https://www.zotero.org/settings/keys} and create a new one. Save the new key in a password manager or note immediately.
